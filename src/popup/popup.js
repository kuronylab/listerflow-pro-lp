...
function updateWorkTimeDisplay(stats, history) {
  const successItems = history.filter(h => {
    const f = h.flags || {};
  ...
  if (statsElements.listingSpeed) {
    ...
    const speedVal = hours > 0 ? (successCount / hours) : 0;
...
async function loadAndDisplayStats() {
  try {
    ...
    if (statsElements.todayListings) statsElements.todayListings.textContent = `${completedCount}件`;
    ...
    updateWorkTimeDisplay(stats, history);
...
async function loadStatistics() {
  ...
  if (now.toDateString() !== lastReset.toDateString()) {
    ...
    if (now.getDay() === 1) stats.weekListings = 0;
...
async function loadHistoryList() {
  ...
  history.forEach((item) => {
    ...
    card.innerHTML = `<div class="history-main"><span>${item.asin}</span></div>`;
